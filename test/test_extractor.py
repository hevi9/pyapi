...
from hevi_lib.processes import cd
import os
import sys
from hevi_lib.tagtree import Forest
import unittest
...
class test_extractor(unittest.TestCase):
    ...
    def test_01(self):
        """ file found """
        with cd(root):
            assert (root, "justamodule.py") in [
                i for i in sut.find_files("justamodule.py")]
    ...
    def test_04(self):
        ...
        files = [i for i in sut.filter_files(
            sut.find_files(j(root, "package")))]
        # D(files)
